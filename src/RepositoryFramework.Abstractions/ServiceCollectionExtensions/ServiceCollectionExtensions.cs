...
namespace Microsoft.Extensions.DependencyInjection
{
    public static partial class ServiceCollectionExtensions
    {
        ...
        /// <summary>
        /// Add Function storage for your CQFRS, inject the IAggregation<<typeparamref name="T"/>, <typeparamref name="TKey"/>>
        /// </summary>
        /// <typeparam name="T">Model used for your query.</typeparam>
        /// <typeparam name="TKey">Key to manage your data from repository.</typeparam>
        /// <typeparam name="TStorage">Function pattern storage.</typeparam>
        /// <param name="services">IServiceCollection.</param>
        /// <param name="serviceLifetime">Service Lifetime.</param>
        /// <param name="notExposableAsApi">It's a parameter used by framework to understand the level of privacy,
        /// used for instance in library Api.Server to avoid auto creation of an api with this repository implementation.</param>
        /// <returns>IRepositoryBuilder<<typeparamref name="T"/>, <typeparamref name="TKey"/>></returns>
        public static IRepositoryBuilder<T, TKey> AddAggregation<T, TKey, TStorage>(this IServiceCollection services,
           ServiceLifetime serviceLifetime = ServiceLifetime.Scoped,
           bool notExposableAsApi = false)
           where TStorage : class, IAggregationPattern<T, TKey>
           where TKey : notnull
        {
            var service = services.SetService<T, TKey>();
            var currentType = typeof(IAggregation<T, TKey>);
            service.AddOrUpdate(currentType, typeof(TStorage));
            service.NotExposableAsApi = notExposableAsApi;
            services
                .RemoveServiceIfAlreadyInstalled<TStorage>(currentType, typeof(IAggregationPattern<T, TKey>))
                .AddService<IAggregationPattern<T, TKey>, TStorage>(serviceLifetime)
                .AddService<IAggregation<T, TKey>, Aggregation<T, TKey>>(serviceLifetime);
            return new RepositoryBuilder<T, TKey>(services, PatternType.Aggregation, serviceLifetime);
        }
    }
}
