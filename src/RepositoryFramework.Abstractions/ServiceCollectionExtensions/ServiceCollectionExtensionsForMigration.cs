...
namespace Microsoft.Extensions.DependencyInjection
{
    public static partial class ServiceCollectionExtensions
    {
        ...
        public static IServiceCollection AddMigration<T, TKey, TStorageToMigrate, TFinalStorage>(this IServiceCollection services,
          ...
          where TStorageToMigrate : class, IRepositoryPattern<T, TKey>
          where TFinalStorage : class, IMigrateRepositoryPattern<T, TKey>
          where TKey : notnull
        {
            var options = new MigrationOptions<T, TKey>();
            settings?.Invoke(options);
            _ = services.AddRepository<T, TKey, TStorageToMigrate>(serviceLifetime)
                .AddSingleton(options);

            return serviceLifetime switch
            {
                ServiceLifetime.Transient => services.AddTransient<IMigrateRepositoryPattern<T, TKey>, TFinalStorage>(),
                ServiceLifetime.Singleton => services.AddSingleton<IMigrateRepositoryPattern<T, TKey>, TFinalStorage>(),
                _ => services.AddScoped<IMigrateRepositoryPattern<T, TKey>, TFinalStorage>()
            };
        }
    }
}